...
- 設問の意図や詳細コンセプトについては`src/data/questionList.json`の各設問の`concept`フィールドを参照。
...
- グループエディター上の一覧は、各行先頭のドラッグハンドルを用いたドラッグ＆ドロップで並べ替えできる。
- 並べ替えの結果は `personalPlotList` の配列順そのものとして保持され、URL共有形式（`p=` の出現順）にもそのまま反映される。
- ドラッグ＆ドロップはデスクトップ操作を前提とし、タッチ操作には対応しない。
- 上記により、狭幅（モバイル）レイアウトではドラッグハンドルを表示しない。可視だが機能しないハンドルを残さないための措置。
